...
According to Newton's second law, force is directly proportional to acceleration keeping mass constant. For the calculation of force on the objects inside the galaxy, we consider it to be a set of concentric rings. The histogram from the above section gives us the mass ditribution of the galaxy as moving through the radius. To find the force on a mass inside and outside a specific ring, let us consider the galaxy to be a collection of concentric rings. The force on a particle at radius 'r' will be the commulative effect of force due to the rings before and after the radius 'r'.
...
\begin{figure}[!tbp]
\centering
\begin{minipage}[b]{0.4\textwidth}[h]
...
\frac{d{\theta}}{\sqrt{R^2 - 2 R \cos \theta + r^2}}$

$ d \theta = 2 d \beta $,
$ \theta = \pi - 2 \beta $,
$ \beta = \frac{\pi - \theta}{2} $

$ \Phi (\vec{r}) = \frac{GMm}{ \pi} \int_{0}^{\pi}  \frac{d \beta }{\sqrt{r^2 + R^2 +2 R r (1 - 2 \sin^2 \beta )}}  $
...
\subsubsection{Function Definition for Calculation of Force}
\begin{verbatim}
def q(r,R) :
...
\subsection{Calculation for Velocity }
(mathematical)
